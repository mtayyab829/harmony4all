export const INVALID_US_PHONE_MESSAGE = 'Enter a valid 10-digit US phone number.';

export function getUSPhoneDigits(raw: string | null | undefined): string {
...
function isTrivialDigitPattern(digits: string): boolean {
...
function hasInvalidNanpCode(threeDigits: string): boolean {
...
export function formatUSPhoneForStorage(raw: string | null | undefined): string {
  const digits = getUSPhoneDigits(raw);
