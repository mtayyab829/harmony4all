...
const PLACEHOLDER_WORDS = new Set([
  ...
  'xxx', 'x', 'na', 'none', 'noname', 'anonymous', 'unknown', 'abc',
...
function isPlaceholder(words: string[]): boolean {
...
export function getNamePartError(
  raw: string | null | undefined,
  { required = true, label = 'Name' }: { required?: boolean; label?: string } = {}
): string | null {
...
export function getFullNameError(
  raw: string | null | undefined,
  { required = true, label = 'Full name' }: { required?: boolean; label?: string } = {}
): string | null {
  const trimmed = normalize(raw)
  if (!trimmed) {
    return required ? `${label} is required` : null
